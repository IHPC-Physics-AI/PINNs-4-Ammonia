...
def calc_reaction_x(T, P, UP, Tf, rho_b, eta):
    ...
    for i in range (0, reac.Nx):
        ...
        Kp_star = -2.691122*np.log10(T[i]) - 5.519265E-5*T[i] + 1.848863E-7*T[i]*T[i] + 2001.6/T[i] + 2.6899
        
        Kp = Kp_star + (0.1191849/T[i] + 91.87212/np.power(T[i],2) + 25122730.0/np.power(T[i],4))*oper.P0/101325.0
        
        Keq = np.power(10.0,Kp)
        
        #beta = (1/Keq)*np.power(P_H2[i],a1)*np.power(P_N2[i],a2)/np.power(P_NH3[i],a3)
        beta = Keq*np.power(P_H2[i],a1)*np.power(P_N2[i],a2)/np.power(P_NH3[i],a3)
        #print('Check value of beta:  ',beta)

        if (T[i] < 748):
            Ri_x[i] = A1*np.exp(-Ea1/T[i]/R)*np.power(P_NH3[i],a41)*np.power(P_H2[i],a51)*(1.0 - np.power(beta,2))
        else: 
            Ri_x[i] = A2*np.exp(-Ea2/T[i]/R)*(P_NH3[i]**a42)*(P_H2[i]**a52)*(1.0 - beta**2) 
