...
def calc_reaction_x(T, P, UP, Tf, rho_b, eta):
    ...
    Ri_x = np.where(P_NH3 < 1.0e-6, 0,
                    np.where(T < 748, A1 * np.exp(-Ea1 / T / R) * np.power(P_NH3, a41) * np.power(P_H2, a51) * (1.0 - np.power(beta, 2)),
                             A2 * np.exp(-Ea2 / T / R) * (P_NH3 ** a42) * (P_H2 ** a52) * (1.0 - beta ** 2)))
    ...
    dCi[reac.Ns*reac.Nx:] = -rho_b * eta * SDHr + UP*(Tf-T)
    return dCi
